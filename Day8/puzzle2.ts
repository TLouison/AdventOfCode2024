...
function isCoordOccupiedBySameNetwork(
  graph: Graph,
  network: string,
  x: number,
  y: number,
): boolean {
  // Set of map and filter to check for any node that has the same x and y
  return (
    graph.networks[network]
      .map((node) => node.x === x && node.y === y)
  ...
  for (var networkIdentifier in graph.networks) {
    ...
    for (const node of network) {
      ...
      for (const edge of node.edges) {
        ...
        let shouldAddAntinodes = true;
        let possibleAntinodes = 0;
        let candidateAntinodes: string[] = [];
        let antiNodeLocationX = node.x + edge.horizontalDistance;
        let antiNodeLocationY = node.y + edge.verticalDistance;
        console.log(`Up next: (${antiNodeLocationX},${antiNodeLocationY})`);

        while (!isOutOfBounds(graph, antiNodeLocationX, antiNodeLocationY)) {
          const coordString: string = `${antiNodeLocationX},${antiNodeLocationY}`;
          if (!foundAntinodes.includes(coordString)) {
            // console.log(
          ...
          antiNodeLocationX += edge.horizontalDistance;
          antiNodeLocationY += edge.verticalDistance;
          console.log(`Up next: (${antiNodeLocationX},${antiNodeLocationY})`);
        }
      }
    }
