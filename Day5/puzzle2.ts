...
//
// Puzzle rules:
// Input is given as two sections:
//  1. Section of X|Y rules, where page X must be printed before page Y
//  2. Pages to print, to be held to the rules of section 1
//
// Page X does not need to be printed immediately before page Y, just some time before it
//
// As a hint, it was suggested to look for the ones that already satisfy the order and skip over them
//
// The desired result is the sum of the middle page numbers of all the valid lists. We do not need to create
// new lists, only check that the ones provided are valid.
...
// Step 1: write parsers to get the data into more usable structures
class Rule {
  pageNum: number;
  beforeList: number[];

  constructor(pageNum: number, before: number) {
    this.pageNum = pageNum;
    this.beforeList = [before];
  }

  addBeforePage(page: number) {
    this.beforeList.push(page);
  }
}

function parseRules(rulesString: string): { [id: number]: Rule } {
  let rules: { [id: number]: Rule } = {};
  for (var line of rulesString.trim().split("\n")) {
    var ruleVals = line.split("|");

    let x: number = parseInt(ruleVals[0]);
    let y: number = parseInt(ruleVals[1]);

    if (x in rules) {
      rules[x].addBeforePage(y);
    } else {
      let newRule = new Rule(x, y);
      rules[x] = newRule;
    }
  }

  console.log(`Parsed ${Object.keys(rules).length} rules to follow.`);
  return rules;
}

function parsePageOrders(orderString: string): number[][] {
  let pageOrders: number[][] = [];

  for (var line of orderString.trim().split("\n")) {
    let orderList = line.split(",").map((val) => parseInt(val));
    pageOrders.push(orderList);
  }
  console.log(`Parsed ${pageOrders.length} page orders to verify.`);
  return pageOrders;
}

// Step two: A function that can check if a list is valid
function isPageOrderValid(
  pageOrder: number[],
  rules: { [id: number]: Rule },
): number {
  // Verifies if the given page order follows the provided rules. If it does, it will return the middle number
  // of the list. If it does not, it will return -1.
  console.log(`Checking page order ${pageOrder}.`);

  // Possible speedup here: feels bad to do an n^2 algorithm for this.
  for (var cur = 1; cur < pageOrder.length; cur++) {
    const currentNum = pageOrder[cur];

    // We only need to check the rules for numbers that have rules associated. Numbers without
    // a rule can be wherever they like so we can just continue.
    if (currentNum in rules) {
      const currentNumRule = rules[currentNum];
      const invalidPages = currentNumRule.beforeList;

      console.log(
        `Checking number ${currentNum}'s list for any invalid ordering. List is ${invalidPages}`,
      );

      // We look backwards to find if any of the pages that preceeded this one breaks any of this number's rules
      for (var prev = cur - 1; prev >= 0; prev--) {
        console.log(`Checking if ${pageOrder[prev]} is invalid.`);
        if (invalidPages.includes(pageOrder[prev])) {
          console.log(`${pageOrder[prev]} is invalid!`);
          return -1;
        }
        console.log(`${pageOrder[prev]} is valid.`);
      }
    } else {
      console.log(`No rules exist for number ${currentNum}. Skipping...`);
      continue;
    }
  }
...
function insertPageAtIndex(
  pageOrder: number[],
  number: number,
  index: number,
): number[] {
  return pageOrder.toSpliced(index);
}

function findValidPageOrder(
  pageOrder: number[],
  rules: { [id: number]: Rule },
): number {
  // Reorders the pages to find a valid order, then returns the middle value of that ordering.
  // This is a variation on a bubble sort, relying on the rules instead of a regular sort
  console.log(
    `BEGIN findValidPageOrder: Attempting to find valid page order for ${pageOrder}.`,
  );

  for (var cur = 0; cur < pageOrder.length; cur++) {
    let currentNum = pageOrder[cur];
    console.log(`Main Loop: Checking ${currentNum}`);
    if (currentNum in rules) {
      const currentNumRule = rules[currentNum];
      const invalidPages = currentNumRule.beforeList;

      // We look backwards to find if any of the pages that preceeded this one breaks any of this number's rules.
      // If they do, we try find the first location it is valid in so we can put it there
      let newPos = cur;
      for (var prev = cur - 1; prev >= 0; prev--) {
        console.log(`Backcheck: Checking if ${pageOrder[prev]} is invalid.`);
        if (invalidPages.includes(pageOrder[prev])) {
          newPos = prev;
        } else {
          break;
      ...
      if (newPos != cur) {
        // Handles swapping the values
        console.log(
          `${pageOrder[cur]} is invalid! Moving it to index ${newPos}`,
        );
        // Remove the value from the old index and add the value to the new index
        let num = pageOrder[cur];
        pageOrder.splice(cur, 1);
        pageOrder.splice(newPos, 0, num);
        console.log(`New ordering is ${pageOrder}.`);

        // Restart each time we swap to ensure we cover all cases
        cur = 0;
      }
    }
  }
  console.log(`Got resulting order ${pageOrder}`);

  return pageOrder[Math.floor(pageOrder.length / 2)];
}

const rules = parseRules(inputRules);
const pageOrders = parsePageOrders(inputPageOrders);

let wellOrderedSum = 0;
let disorderedSum = 0;
for (var pageOrder of pageOrders) {
  let result = isPageOrderValid(pageOrder, rules);
  if (result > 0) {
    wellOrderedSum += result;
  } else {
    disorderedSum += findValidPageOrder(pageOrder, rules);
  }
}
console.log(wellOrderedSum);
console.log(disorderedSum);
console.log(wellOrderedSum + disorderedSum);
